...
const create = () => {
  ...
  colors.symbols = __nccwpck_require__(522);
...
const ansi_colors_1 = __importDefault(__nccwpck_require__(151));
function main() {
    try {
        const secretNames = core.getMultilineInput("names");
        if (!secretNames) {
            core.setFailed("No value provided for required input: names");
            return;
        }
        if (secretNames.length === 0 || secretNames.every((name) => name.length === 0)) {
            core.warning(ansi_colors_1.default.yellow("Skipping secret check, no names were provided"));
            return;
        }
        core.info(ansi_colors_1.default.whiteBright("Checking availability of secrets..."));
        let allExist = true;
        secretNames.forEach((name) => {
            if (name in process.env && process.env[name].length > 0) {
                core.info(ansi_colors_1.default.green(`  ✓ ${name}`));
            }
            else {
                core.info(ansi_colors_1.default.red(`  ✗ ${name}`));
                allExist = false;
            }
        });
        if (!allExist) {
            core.error(ansi_colors_1.default.red("If dependabot initiated the workflow, make sure to add the missing secrets to Dependabot's secrets."));
            core.error(ansi_colors_1.default.red("More information: https://docs.github.com/en/code-security/dependabot/working-with-dependabot/configuring-access-to-private-registries-for-dependabot"));
